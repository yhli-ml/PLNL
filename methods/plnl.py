...
parser = argparse.ArgumentParser(
	prog='complementary label learning with PLG and NLE',
...
parser.add_argument('-pre_eps', default=200, type=int, metavar='N', help='number of pre-training epochs using SCL-LOG')
...
def train(train_loader, model, optimizer, epoch, consistency_criterion, flag, ori_comp_labels_matrix):
    ...
    # negative label enhancement (NLE)
    if epoch >= args.weps:
...
def training_pipeline():
    ...
    if args.model == 'resnet':
        if args.dataset in ['cifar10', 'cifar100', 'svhn', 'clcifar10', 'clcifar20']:
            ...
            model_t, model_s = ResNet18_mnist(num_classes=num_classes),ResNet18_mnist(num_classes=num_classes)
    ...
    if not os.path.exists(model_path):
        ...
        for epoch in range(args.pre_eps):
            pretrain(args, train_loader_t, model, optimizer, epoch, test_loader)
            scheduler.step()
    model_t.load_state_dict(torch.load(model_path))
